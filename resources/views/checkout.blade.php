...
<div class="bg-white">
    <main class="pt-24 relative grid grid-cols-1 gap-x-16 max-w-7xl mx-auto lg:px-8 lg:grid-cols-2">
      ...
      <section aria-labelledby="summary-heading" class="text-indigo-300 pt-6 pb-12 md:px-10 lg:max-w-lg lg:w-full lg:mx-auto lg:px-0 lg:pt-0 lg:pb-24 lg:bg-transparent lg:row-start-1 lg:col-start-2">
          ...
          <ul role="list" class="text-sm font-medium divide-y divide-white divide-opacity-10">
            ...
            @foreach($cart as $item)
                <li class="flex items-start py-6 space-x-4 text-black">
                    <img src="@if(isset($item->product->image)){{$item->product->image}}@else{{asset('/images/other/notfound.jpg')}}@endif" alt="Front of zip tote bag with white canvas, white handles, and black drawstring top." class="flex-none w-20 h-20 rounded-md object-center object-cover">
                    ...
                    @else
                        ...
                        <p class="text-xl font-bold text-gray-900">{{ number_format($item->product->price_student, 0, '', ' ') }} ₽</p>
                    @endif
                    </p>
                </li>
            @endforeach
