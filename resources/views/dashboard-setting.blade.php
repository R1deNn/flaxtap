        ...
        <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-[#ededed]">
            ...
            <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-[#ffffff] lg:translate-x-0 lg:static lg:inset-0">
                ...
                <nav class="mt-10">
                    ...
                    <a class="flex items-center px-6 py-2 mt-4 text-gray-500 hover:bg-[#5e5e5e] duration-300 hover:bg-opacity-25"
                       href="{{route('/dashboard-your-orders')}}">
                    ...
                    <a class="flex items-center px-6 py-2 mt-4 text-white bg-[#5e5e5e]"
            ...
            <div class="flex flex-col flex-1 overflow-hidden rounded">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto rounded">
                    <div class="container px-6 py-8 mx-auto rounded">
                        <h3 class="text-3xl font-medium text-gray-700">Настройки</h3>
                            ...
                            <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 rounded">
                                <div
                                    ...
                                    <div class="col-span-8 overflow-hidden rounded-xl sm:bg-gray-50 sm:px-8 sm:shadow">
                                        <hr class="mt-4 mb-8" />
                                        <p class="py-2 text-xl font-semibold">Электронная почта</p>
                                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                            <p class="text-gray-600">Ваша почта <strong>{{auth()->user()->email}}</strong></p>
                                        </div>

                                        <hr class="mt-4 mb-8" />

                                        <p class="text-xl font-semibold">Ваши сертификаты</p>
                                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                                    ...
                                                    <div class="swiper-wrapper">
                                                        @foreach($certificates as $certificate)
                                                            <div class="swiper-slide">
                                                                <img class="w-50" src="{{asset('storage/' . $certificate->path . $certificate->name . '.' . $certificate->extension)}}" alt="certificate">
                                                                <form action="{{route('remove-certificate')}}" method="POST">
                                                                    @csrf
                                        ...
                                        <p class="mt-4">Загрузить новый</p>

                                        <form action="{{route('upload-certificate')}}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            <input type="file" accept="image/*" name="user_certificate">
                                            <br>
                                            <x-primary-button class="mt-4" type="submit">Загрузить</x-primary-button>
                                        </form>

                                        <hr class="mt-4 mb-8" />
                                        <p class="py-2 text-xl font-semibold">Пароль</p>
                                        <div class="flex items-center">
                                            <div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-3">
                                                <form action="{{route('dashboard.password.update')}}" method="POST">
                                                    @csrf
                                                    <label for="login-password">
                                                        <span class="text-sm text-gray-500">Текущий пароль</span>
                                                        <div class="relative flex overflow-hidden rounded-md border-2 transition focus-within:border-blue-600">
                                                            <input name="password" type="password" id="login-password" class="w-full flex-shrink appearance-none border-gray-300 bg-white py-2 px-4 text-base text-gray-700 placeholder-gray-400 focus:outline-none" placeholder="***********" />
                                                        </div>
                                                    </label>
                                                    <label for="login-password">
                                                        <span class="text-sm text-gray-500">Новый пароль</span>
                                                        <div class="relative flex overflow-hidden rounded-md border-2 transition focus-within:border-blue-600">
                                                            <input name="password_confirmation" type="password" id="login-password" class="w-full flex-shrink appearance-none border-gray-300 bg-white py-2 px-4 text-base text-gray-700 placeholder-gray-400 focus:outline-none" placeholder="***********" />
                                                        </div>
                                                    </label>
                                                    ...
                                                    <button class="mt-4 rounded-lg bg-blue-600 px-4 py-2 text-white">Сохранить</button>
                                                </form>
                                            </div>
                                        </div>
                                        <hr class="mt-4 mb-8" />
                                        ...
                                        <div class="mb-10">
                                            <p class="py-2 text-xl font-semibold">ФИО</p>
                                            <div class="flex items-center">
                                                <div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-3">
                                                    <label for="login-password">
                                                        <span class="text-sm text-gray-500">Текущее ФИО</span>
                                                        <div class="relative flex overflow-hidden rounded-md border-2 transition focus-within:border-blue-600">
                                                            <input type="text" id="login-password" class="w-full flex-shrink appearance-none border-gray-300 bg-white py-2 px-4 text-base text-gray-700 placeholder-gray-400 focus:outline-none" value="{{auth()->user()->fio}}" />
                                                        </div>
                                                    </label>
                                                </div>
                                            </div>
                                        ...
                                        <div class="mb-10">
                                            <p class="py-2 text-xl font-semibold">Номер телефона</p>
                                            <div class="flex items-center">
                                                <div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-3">
                                                    <label for="login-password">
                                                        <span class="text-sm text-gray-500">Текущий номер телефона</span>
                                                        <div class="relative flex overflow-hidden rounded-md border-2 transition focus-within:border-blue-600">
                                                            <input type="text" id="login-password" class="w-full flex-shrink appearance-none border-gray-300 bg-white py-2 px-4 text-base text-gray-700 placeholder-gray-400 focus:outline-none" value="{{auth()->user()->tel}}" />
                                                        </div>
                                                    </label>
                                                </div>
                                            </div>
