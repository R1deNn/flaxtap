...
class ShopEditScreen extends Screen
{
     ...
     * @return array
     */
    public function query(Shop $shop): array
    {
        $shop->load('attachment');
    ...
    {
        $shop
            ->fill($request->collect('shop')->except(['shop.id_category', 'shop.id_vobler', 'shop.only_trainer', 'shop.title', 'shop.description', 'shop.default_price',
                'shop.price_student', 'shop.price_opt_student', 'shop.amount', 'shop.image'])->toArray())
            ->save();

        $shop->attachment()->syncWithoutDetaching(
            $request->input('shop.attachment', [])
        );
