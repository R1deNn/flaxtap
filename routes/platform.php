...
// Platform > Gifts
Route::screen('/gifts', \App\Orchid\Screens\GiftsScreen::class)
    ->name('platform.gifts')
    ->breadcrumbs(function (Trail $trail) {
        $trail->push(__('Подарки'), route('platform.gifts'));
    });

// Platform > Gifts > Create
Route::screen('gift/create', \App\Orchid\Screens\GiftEditScreen::class)
    ->name('platform.gift.create');

// Platform > Gifts > Gift
Route::screen('gift/{gift}/edit', \App\Orchid\Screens\GiftEditScreen::class)
    ->name('platform.gift.edit')
    ->breadcrumbs(fn (Trail $trail, $gift) => $trail
        ->parent('platform.gifts')
        ->push($gift->id, route('platform.gift.edit', $gift)));
...
// Platform > Orders > Check
Route::screen('order/{order}/check', \App\Orchid\Screens\OrderCheckScreen::class)
    ->name('platform.order.check')
    ->breadcrumbs(fn (Trail $trail, $order) => $trail
        ->parent('platform.orders'));
